...
export default function CentroNotificaciones({ isOpen, onClose }) {
    ...
    const cargarAlertas = async () => {
        ...
        try {
            ...
            // 3. EVALUAR PERCANCES RECIENTES (Últimos 3 días)
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-3">
                            {alertas.map(alerta => {
                                ...
                                if (alerta.prioridad === 'ALTA') colorClass = 'text-red-600 bg-red-50 border-red-200'; // Override si es urgente

                                return (
                                    <div key={alerta.id} className={`p-4 rounded-2xl border bg-white shadow-sm flex gap-4 ${alerta.prioridad === 'ALTA' ? 'border-l-4 border-l-red-500' : 'border-l-4 border-l-transparent'}`}>
