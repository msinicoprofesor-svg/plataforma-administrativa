/* -------------------------------------------------------------------------- */
/* ARCHIVO: app/components/Cobertura.tsx (OFERTA COMERCIAL AVANZADA)          */
/* -------------------------------------------------------------------------- */
'use client';
import { useState } from 'react';
import { 
  MdMap, MdAdd, MdSearch, MdWifi, MdCable, MdClose, MdRouter, 
  MdLocationOn, MdDelete, MdPlace, MdBusiness, MdDomain, MdAttachMoney, MdSpeed, MdCheckCircle, MdWarning, MdSwapHoriz
} from "react-icons/md";
...
export default function Cobertura({ cobertura, onAgregarZona, onActualizarZona, usuarioActual }) {
  ...
  const agregarCajaALista = () => {
    ...
    setNuevaCaja({ nombre: '', calles: '', puertos: 8 }); 
  };
  ...
  const guardarConfiguracionMarketing = (e) => {
      ...
      const zonaActualizada = {
          ...zonaAConfigurar,
          costos: { 
              instalacion: Number(costos.instalacion), 
              cambio: Number(costos.cambio) 
          },
          planes: planes,
          estatus: 'ACTIVA'
      };
  ...
  const handleGuardarZona = (e) => {
    ...
    const zonaFinal = {
        ...
        cajas: tipoTecnologia === 'FIBRA' ? cajasTemporales.map(c => ({
            ...
            puertosLibres: Number(c.puertos) 
        })) : [],
        ...
        estatus: 'PENDIENTE_PRECIOS',
        costos: { instalacion: 0, cambio: 0 },
        planes: []
    };
  ...
  const zonasFiltradas = cobertura.filter(z => 
    z.comunidad?.toLowerCase().includes(busqueda.toLowerCase()) || 
    z.municipio?.toLowerCase().includes(busqueda.toLowerCase()) ||
    z.sede?.toLowerCase().includes(busqueda.toLowerCase())
  );

  return (
    <div className="h-full flex flex-col">
      {/* HEADER */}
      <div className="bg-white p-6 rounded-[2rem] shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 mb-6 shrink-0">
        <div>
            <h2 className="text-xl font-extrabold text-gray-800 flex items-center gap-2"><MdMap className="text-blue-500" /> Cobertura Técnica</h2>
            <p className="text-sm text-gray-400 font-medium">Infraestructura y Tarifas</p>
        </div>
        <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
                <MdSearch className="absolute left-4 top-3.5 text-gray-400 text-xl" />
                <input type="text" placeholder="Buscar zona..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-gray-50 rounded-2xl border-none outline-none focus:ring-2 focus:ring-blue-100 text-gray-700 font-medium" />
            </div>
            <button onClick={() => setModalOpen(true)} className="px-6 py-3 bg-blue-600 text-white font-bold rounded-2xl shadow-lg hover:bg-blue-700 transition-all flex items-center justify-center gap-2"><MdAdd className="text-xl" /> Nueva Zona</button>
        </div>
      </div>
            ...
            <div className="bg-white rounded-[2rem] w-full max-w-lg p-8 shadow-2xl animate-scale-in max-h-[90vh] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                    ...
                    <div className="bg-gray-50 p-4 rounded-2xl mb-6 border border-gray-100">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-[10px] font-bold text-green-600 uppercase mb-1">Instalación Nueva ($)</label>
                                <input type="number" placeholder="Ej: 1500" value={costos.instalacion} onChange={(e) => setCostos({...costos, instalacion: e.target.value})} className="w-full px-4 py-3 bg-white rounded-xl outline-none font-bold text-gray-800 border border-gray-200 focus:border-green-400" />
                            </div>
                            <div>
                                <label className="block text-[10px] font-bold text-blue-600 uppercase mb-1">Cambio Prov. ($)</label>
                                <input type="number" placeholder="Ej: 500" value={costos.cambio} onChange={(e) => setCostos({...costos, cambio: e.target.value})} className="w-full px-4 py-3 bg-white rounded-xl outline-none font-bold text-gray-800 border border-gray-200 focus:border-blue-400" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex gap-2 mb-3">
                            <input type="text" placeholder="Ej: 50" value={nuevoPlan.velocidad} onChange={(e) => setNuevoPlan({...nuevoPlan, velocidad: e.target.value})} className="w-24 px-4 py-2 bg-gray-50 rounded-xl text-sm outline-none border border-gray-200" />
                            <span className="self-center font-bold text-gray-400 text-xs">MB</span>
                            <span className="self-center font-bold text-gray-400 text-sm ml-2">$</span>
                            <input type="number" placeholder="Precio" value={nuevoPlan.precio} onChange={(e) => setNuevoPlan({...nuevoPlan, precio: e.target.value})} className="flex-1 px-4 py-2 bg-gray-50 rounded-xl text-sm outline-none border border-gray-200" />
                            <button type="button" onClick={agregarPlan} className="px-4 py-2 bg-blue-600 text-white rounded-xl font-bold text-sm hover:bg-blue-700">Agregar</button>
                        </div>
                        ...
                        <div className="space-y-2">
                            {planes.map((p) => (
                                <div key={p.id} className="flex justify-between items-center bg-white border border-gray-100 p-3 rounded-xl shadow-sm">
                                    <div>
                                        <p className="font-extrabold text-gray-800 text-sm flex items-center gap-2"><MdSpeed className="text-blue-500"/> {p.velocidad}</p>
                                        <p className="text-xs text-gray-500 font-bold">${p.precio} / mes</p>
                                    </div>
                                    <button onClick={() => eliminarPlan(p.id)} className="text-red-400 hover:text-red-600 p-2"><MdDelete/></button>
                                </div>
                            ))}
