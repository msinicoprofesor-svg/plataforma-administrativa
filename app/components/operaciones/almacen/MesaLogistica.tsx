...
export default function MesaLogistica({ useData, colaboradores }) {
    ...
    const getNombreBase = (str) => {
        if (!str) return '';
        const lastParen = str.lastIndexOf(' (');
        return lastParen !== -1 ? str.substring(0, lastParen).trim() : str.trim();
    };

    // ESCANEO EXACTO DE ALMACÉN GENERAL (Ignorando la sucursal Centro)
    const getStockGeneral = (productoIdFull) => {
        const nombreBase = getNombreBase(productoIdFull);
        const fisicos = inventario.filter(p => 
            p.nombre === nombreBase && 
            (p.almacen === 'ALMACÉN GENERAL' || p.almacen === 'ALMACEN GENERAL' || p.region === 'Almacén General')
        );
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden relative">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5 bg-gray-50/30">
                {solicitudes.length === 0 ? (
                    <div className="text-center py-20 text-gray-400 font-bold">No hay solicitudes.</div>
                ) : (
