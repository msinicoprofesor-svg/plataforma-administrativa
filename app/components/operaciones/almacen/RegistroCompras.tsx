...
import { MdShoppingCart, MdAdd, MdDelete, MdSave, MdFileUpload, MdCheckCircle, MdHistory } from "react-icons/md";
...
const REGIONES_DISPONIBLES = ['Almacén General', 'Centro', 'Comonfort', 'Tlalpujahua', 'Gandhó', 'San Diego de la Unión', 'Amealco', 'Xichú', 'Jalpan de Serra', 'Santa María del Río', 'WifiCel', 'RK', 'Fibrox MX', 'Intercheap'];

export default function RegistroCompras({ useData, usuarioActivo, colaboradores = [] }) {
    ...
    const handleCambioItem = (index, campo, valor) => { const nuevos = [...items]; nuevos[index][campo] = valor; setItems(nuevos); };
    ...
    const handleGuardar = async (e) => {
        ...
        try {
            const payload = { proveedor, factura_url: archivoBase64, total_articulos: itemsValidos.reduce((acc, el) => acc + parseInt(el.cantidad), 0), usuario_registro_id: usuarioActivo?.id || 'SISTEMA' };
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden">
            <div className="p-5 border-b border-gray-100 bg-gray-50/50 shrink-0 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Sube la factura y asigna los productos a una marca y ubicación</p>
                </div>
                <button onClick={() => setModalHistorial(true)} className="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-full font-black text-xs transition-all shadow-sm active:scale-95 flex items-center justify-center gap-2 shrink-0">
                    <MdHistory className="text-lg"/> Historial de Compras
                </button>
            </div>
                            ...
                            {items.map((item, idx) => (
                                <div key={idx} className="flex flex-wrap md:flex-nowrap gap-3 items-center bg-white p-3 rounded-xl border border-blue-100 shadow-sm relative">
                                    ...
                                    <div className="w-1/2 md:w-36">
                                        <select required disabled={!esAdminGeneral} value={item.marca} onChange={e=>handleCambioItem(idx, 'marca', e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-2 py-2 text-[10px] font-black text-gray-700 outline-none uppercase">
                                            {marcasPermitidas.map(m => <option key={m} value={m}>{m}</option>)}
                                        </select>
                                    </div>
                                    <div className="w-1/2 md:w-36">
                                        <select required disabled={!esAdminGeneral} value={item.region} onChange={e=>handleCambioItem(idx, 'region', e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-2 py-2 text-[10px] font-black text-gray-700 outline-none uppercase">
                                    ...
                                    <button type="button" onClick={()=>handleQuitarItem(idx)} className="w-8 h-8 flex items-center justify-center text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg absolute md:relative top-2 right-2 md:top-auto md:right-auto"><MdDelete className="text-xl"/></button>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <ModalHistorialGlobal isOpen={modalHistorial} onClose={() => setModalHistorial(false)} useData={useData} usuarioActivo={usuarioActivo} colaboradores={colaboradores} contextoInicial="COMPRAS" />
        </div>
    );
}