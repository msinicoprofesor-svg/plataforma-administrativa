...
export default function PortalSolicitudes({ useData, usuarioActivo }) {
    ...
    const getSugerencias = (productoId) => {
        ...
        switch (prod.categoria) {
            case 'FIBRA ÓPTICA': case 'CABLEADO': return ["Tendido de nueva ruta", "Reparación de corte de fibra", "Instalación de cliente nuevo"];
            case 'ENLACE / ANTENA': case 'REDES': return ["Instalación de cliente", "Cambio de tecnología", "Reubicación de equipo", "Falla por descarga eléctrica"];
            case 'HERRAMIENTA': case 'EQUIPO': return ["Asignación a cuadrilla nueva", "Reposición por desgaste", "Extravío / Robo"];
            case 'CCTV': return ["Instalación de cámaras", "Mantenimiento a DVR", "Reubicación de cámara"];
            default: return ["Uso en oficina", "Mantenimiento general", "Stock de seguridad para la región"];
        }
    };

    const sugerenciasDinamicas = getSugerencias(form.productoId);
    const handleSugerencia = (texto) => setForm(prev => ({ ...prev, motivo: prev.motivo ? `${prev.motivo} - ${texto}` : texto }));
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = { usuario_solicitante_id: usuarioActivo.id, destino: form.destino, motivo: form.motivo };
        const detalles = [{ producto_id: productoSeleccionado.nombre, cantidad_solicitada: form.cantidad }];
                ...
                {estado === 'CANCELADO' ? (
                    ...
                    <>
                        <div className="flex justify-between items-center relative mb-2">
                            ...
                            {pasos.map((paso, index) => <div key={paso} className={`w-4 h-4 rounded-full border-2 ${index <= pasoActual ? 'bg-blue-600 border-blue-600' : 'bg-white border-gray-300'} transition-all duration-500`}></div>)}
                        </div>
                        <div className="flex justify-between text-[8px] font-black text-gray-400 uppercase">
                            <span>Pendiente</span><span>Comprado</span><span>En Almacén</span><span>En Camino</span><span className={estado === 'ENTREGADO' ? 'text-green-600' : ''}>Entregado</span>
                        </div>
                    </>
                )}
        ...
        <div className="h-full flex flex-col xl:flex-row gap-6 animate-fade-in pb-10">
            <div className="xl:w-[45%] bg-white p-6 rounded-3xl shadow-sm border border-gray-100 shrink-0 overflow-y-auto custom-scrollbar">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase mb-2 ml-1 flex items-center gap-1"><MdLightbulbOutline className="text-yellow-500"/> Sugerencias</label>
                        <div className="flex flex-wrap gap-2 mb-3 min-h-[30px]">
                            {sugerenciasDinamicas.map(sug => (
                                <button type="button" key={sug} onClick={() => handleSugerencia(sug)} className="px-3 py-1.5 bg-white border border-gray-200 text-gray-600 hover:border-blue-300 hover:text-blue-600 rounded-full text-[10px] font-bold transition-all shadow-sm active:scale-95">{sug}</button>
                            ))}
                        </div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase mb-2 ml-1">Motivo Final *</label>
                        <textarea required value={form.motivo} onChange={e => setForm({...form, motivo: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm font-bold text-gray-800 outline-none focus:border-blue-500 h-24 resize-none"></textarea>
                    </div>
                    <button type="submit" disabled={isSubmitting} className="w-full bg-gray-900 hover:bg-black text-white font-black py-4 rounded-2xl transition-all shadow-xl shadow-gray-900/20 disabled:opacity-50 flex justify-center items-center gap-2 active:scale-95 mt-4">
                        <MdSend className="text-lg"/> Enviar Solicitud
                    </button>
                </form>
            </div>
