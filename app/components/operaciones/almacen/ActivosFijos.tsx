...
export default function ActivosFijos({ useData, usuarioActivo, colaboradores = [] }) {
    ...
    const activosFiltrados = activosPermitidos.filter(a => {
        ...
        return (
            ...
            (a.numero_serie && a.numero_serie.toLowerCase().includes(q)) ||
            (a.marca && a.marca.toLowerCase().includes(q)) ||
            nombreResponsable.includes(q)
        );
    });
    ...
    const handleGuardar = async (e) => {
        ...
        if (res.success) {
            ...
            alert("❌ Ocurrió un error al guardar el activo en la base de datos.");
        }
    };
            ...
            <div className="p-5 border-b border-gray-100 bg-gray-50/50 shrink-0 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex w-full md:w-auto gap-3">
                    <div className="relative flex-1 md:w-72">
                        ...
                        <input type="text" placeholder="Buscar por serie, nombre o responsable..." value={busqueda} onChange={e => setBusqueda(e.target.value)} className="w-full bg-white border border-gray-200 rounded-full pl-11 pr-4 py-2.5 text-xs font-bold text-gray-800 outline-none focus:border-blue-500 shadow-sm" />
                    </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
                {cargando ? (
                    ...
                    <div className="text-center py-20">
                        <MdComputer className="text-5xl text-gray-200 mx-auto mb-3" />
                        <p className="text-gray-400 font-bold">No hay activos fijos registrados con estos filtros.</p>
                    </div>
                ) : (
                                    ...
                                    <tr key={a.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="p-4">
                                            ...
                                            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">
                                                {a.categoria.replace('_', ' ')} • {a.marca || 'Sin marca'}
                                            </p>
                                        </td>
                                        <td className="p-4">
                                            {a.numero_serie ? (
                                                <span className="text-[10px] font-black text-gray-600 font-mono bg-gray-100 px-2 py-1 rounded border border-gray-200">
                                                    {a.numero_serie}
                                                </span>
                                            ) : (
                                                <span className="text-[9px] font-bold text-gray-300 italic">S/N</span>
                                            )}
                                        </td>
                                        <td className="p-4">
                                            <span className="text-xs font-bold text-gray-800">{a.region}</span>
                                        </td>
                                        <td className="p-4">
                                            {colaborador ? (
                                                <div className="flex items-center gap-2">
                                                    <div className="w-6 h-6 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-black text-[10px] shrink-0">
                                                        {colaborador.nombre.charAt(0)}
                                                    </div>
                                                    <div>
                                                        <p className="text-[10px] font-black text-gray-700">{colaborador.nombre}</p>
                                                        <p className="text-[8px] font-bold text-gray-400 uppercase">{colaborador.puesto}</p>
                                                    </div>
                                                </div>
                                            ) : (
                                        ...
                                        <td className="p-4 text-center">
                                            <span className={`text-[9px] font-black px-2 py-1.5 rounded-lg border uppercase tracking-widest
                                                ${a.estado === 'ACTIVO' ? 'bg-green-50 text-green-700 border-green-200' : 
                                                  a.estado === 'EN_REPARACION' ? 'bg-orange-50 text-orange-700 border-orange-200' : 
                    ...
                    <form onSubmit={handleGuardar} className="bg-white rounded-[2rem] w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-slide-up">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 shrink-0">
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-gray-400 uppercase mt-1 tracking-widest">Asigna equipo de alto valor a una persona/región</p>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-5">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-gray-500 uppercase mb-2 ml-1">Ubicación Física (Almacén/Región) *</label>
                                ...
                                <div className="md:col-span-2 border-t border-gray-100 pt-5 mt-2">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-black text-gray-500 uppercase mb-2 ml-1 flex items-center gap-1"><MdStickyNote2 className="text-gray-400"/> Notas / Observaciones</label>
                                            <input type="text" value={nuevoActivo.notas} onChange={e => setNuevoActivo({...nuevoActivo, notas: e.target.value})} placeholder="Detalles de entrega, accesorios..." className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold text-gray-800 outline-none focus:border-blue-500" />
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="p-6 border-t border-gray-100 bg-gray-50 flex gap-3 shrink-0">
                            ...
                            <button type="submit" disabled={isSubmitting} className="flex-1 bg-gray-900 hover:bg-black text-white font-black py-4 rounded-xl transition-all shadow-lg shadow-gray-900/20 active:scale-95 flex justify-center items-center gap-2">
                                {isSubmitting ? <div className="w-5 h-5 border-2 border-gray-400 border-t-white rounded-full animate-spin"></div> : <MdAdd className="text-lg"/>} 
                                Registrar Activo
                            </button>
                        </div>
                    </form>
