...
export default function GestionRutas() {
    ...
    return (
        <div className="h-full flex flex-col space-y-6 animate-fade-in pb-10 relative">
            ...
            {(loadingTickets || loadingColabs || isUpdating) && (
                <div className="absolute inset-0 bg-[#F5F7FA]/70 backdrop-blur-sm z-20 flex flex-col items-center justify-center rounded-3xl">
                    ...
                    <p className="text-xs font-bold text-gray-700 mt-3 animate-pulse">
                        {isUpdating ? 'Actualizando base de datos...' : 'Sincronizando sistema...'}
                    </p>
                </div>
            )}

            {/* BARRA SUPERIOR (CALENDARIO Y VISTAS) */}
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex bg-gray-100 p-1.5 rounded-2xl border border-gray-200/50 shadow-inner">
                        <button 
                            ...
                            className={`p-2 rounded-xl transition-all ${vistaActiva === 'TABLERO' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                            title="Modo Tablero (Drag & Drop)"
                        >
                            <MdViewColumn className="text-lg"/>
                        </button>
                        <button 
                            onClick={() => setVistaActiva('TIMELINE')}
                            className={`p-2 rounded-xl transition-all ${vistaActiva === 'TIMELINE' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
            ...
            {vistaActiva === 'TABLERO' ? (
                /* --- VISTA 1: TABLERO KANBAN (DRAG & DROP) --- */
                    ...
                    >
                        <div className="flex items-center justify-between mb-5 px-2">
                            ...
                            <span className="bg-white text-gray-800 text-[10px] font-black px-2.5 py-1 rounded-lg shadow-sm border border-gray-100">
                                {ticketsDelDia.filter(t => !t.asignadoA || t.asignadoA === 'pendientes').length}
                            </span>
                        </div>
                        ...
                        <div className="flex-1 space-y-4 overflow-y-auto overflow-x-hidden custom-scrollbar pr-2 pb-2">
                            {ticketsDelDia.filter(t => !t.asignadoA || t.asignadoA === 'pendientes').map(ticket => (
                                <TicketCard 
                                    key={ticket.id} ticket={ticket} onDragStart={handleDragStart} 
                                    getColores={getColoresPrioridad} onVerDetalles={abrirDetalles} onReprogramar={reprogramarTicket} 
                                />
                            ))}
                        </div>
                    </div>
                            ...
                            >
                                <div className="flex items-start gap-4 mb-5 p-4 bg-blue-50/50 rounded-[1.5rem] border border-blue-100/50">
                                    ...
                                    <span className="bg-white text-blue-800 text-[10px] font-black px-2 py-1 rounded-lg shadow-sm">
                                        {ticketsDelDia.filter(t => t.asignadoA === tecnico.id).length}
                                    </span>
                                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar px-1 space-y-6">
                    ...
                    {tecnicos.every(tecnico => ticketsDelDia.filter(t => t.asignadoA === tecnico.id).length === 0) && (
                        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[2rem] border border-gray-100 shadow-sm">
                            ...
                            <p className="text-sm font-black text-gray-400 uppercase tracking-widest">No hay rutas activas para esta fecha</p>
                        </div>
                    )}
                </div>
            )}
        ...
        >
            ...
            <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
                ...
                <div className="relative overflow-hidden group/date cursor-pointer">
                    <input 
                        type="date" 
                        value={ticket?.fecha_agendada}
                        onChange={(e) => onReprogramar(ticket.id, e.target.value)}
                        className="absolute opacity-0 inset-0 cursor-pointer z-10"
                        title="Agendar / Posponer ticket"
                    />
                ...
                <button 
                    ...
                    className="text-[9px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-wider transition-colors z-10 flex items-center gap-1"
                >
